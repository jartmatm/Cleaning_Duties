alter table public.sites
  add column if not exists time_zone text not null default 'Australia/Melbourne';
...
begin
  ...
  select coalesce(nullif(btrim(s.time_zone), ''), 'Australia/Melbourne')
  into v_site_time_zone
  from public.sites s
  where s.id = v_duty.site_id;

  v_site_time_zone := coalesce(v_site_time_zone, 'Australia/Melbourne');
  ...
  if not v_duty.recurring then
    if v_duty.due_date is not null
      and v_duty.due_date <= now()
      and v_duty.status::text in ('Draft', 'Scheduled', 'Pending', 'In Progress') then
      update public.cleaning_duties
      set status = case
  ...
  loop
    if v_pattern = 'weekly' then
      ...
      where exists (
        ...
        where weekday.value::integer = extract(dow from (v_next_start + day_offset * interval '1 day') at time zone v_site_time_zone)::integer
      );
  ...
  if v_duty.due_date is null or v_duty.due_date > now() then
    if v_duty.status::text = 'Scheduled'
      and (v_next_start at time zone v_site_time_zone)::date <= (now() at time zone v_site_time_zone)::date then
