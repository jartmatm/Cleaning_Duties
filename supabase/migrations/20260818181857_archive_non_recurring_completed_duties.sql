...
begin
  ...
  if not v_duty.recurring then
    if v_duty.status::text in ('Draft', 'Scheduled', 'Pending', 'In Progress', 'Completed') then
      update public.cleaning_duties
      set status = case
...
revoke all on function public.advance_duty_schedule(uuid) from public, anon;
grant execute on function public.advance_duty_schedule(uuid) to authenticated, service_role;

-- Preserve completed_at so retention still starts when the work was completed.
update public.cleaning_duties
set status = 'Archived'::public.duty_status,
    updated_at = now()
where not recurring
