...
export function SiteSwitcher({ companyName, activeSite, sites, onSelectSite }: SiteSwitcherProps) {
  ...
  return (
    <Card className="relative flex items-center justify-between gap-4 p-4">
      ...
      <button
        className="inline-flex items-center gap-2 rounded-md bg-slate-100 px-3 py-2 text-sm font-medium text-slate-700"
        type="button"
      ...
      {open && sites.length > 0 ? (
        <div className="absolute left-4 right-4 top-[calc(100%-0.25rem)] z-20 mt-2 rounded-lg border border-slate-200 bg-white p-2 shadow-xl shadow-slate-200/70">
          {sites.map((site) => (
            <button
              key={site.id}
              type="button"
              className="flex w-full items-center justify-between rounded-md px-4 py-3 text-left text-sm transition hover:bg-slate-50"
              onClick={() => {
