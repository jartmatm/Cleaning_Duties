...
export function AppLogo({ title = "Cleaning Duties", subtitle = "Multi-site operations", logoUrl }: AppLogoProps) {
  return (
    <div className="inline-flex items-center gap-3">
      {logoUrl ? (
        <img src={logoUrl} alt={`${title} logo`} className="h-10 w-10 rounded-2xl border border-[var(--company-border)] object-cover" />
      ) : (
        <div className="h-10 w-10 rounded-2xl bg-[var(--company-primary)]" />
      )}
      <div>
