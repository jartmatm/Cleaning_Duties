...
export const ToggleBase = ({ className, isHovered, isDisabled, isFocusVisible, isSelected, slim, size = "sm" }: ToggleBaseProps) => {
    ...
    return (
        <div
            className={cx(
                "cursor-pointer rounded-full bg-slate-200 ring-[0.5px] ring-slate-300 outline-slate-900 transition duration-150 ease-linear ring-inset",
                isSelected && "bg-slate-900",
                isSelected && isHovered && "bg-slate-800",
        ...
        >
            <div
                ...
                className={cx(
                    ...
                    slim && "shadow-xs",
                    slim && "border border-slate-300",
                    slim && isSelected && "border-white",
                    slim && isSelected && isHovered && "border-white",
                ...
                <>
                    ...
                    {(label || hint) && (
                        <div className={cx("flex flex-col", styles[size].textWrapper)}>
                            {label && <p className={cx("select-none text-slate-700", styles[size].label)}>{label}</p>}
                            {hint && (
                                <span className={cx("text-slate-500", styles[size].hint)} onClick={(event) => event.stopPropagation()}>
                                    {hint}
                                </span>
                            )}
                        </div>
